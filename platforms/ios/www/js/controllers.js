 ...
 */
    .controller('RemindsCtrl', function ($scope, $state, Reminds) {
        ...
        for(i=0; i<$scope.reminds.length; i++){
            $scope.reminds[i].timestamp = new Date($scope.reminds[i].time).getTime();
...
//                $scope.reminds[i].notExpired = true;
        ...
        $scope.clearExpired = function() {
            ...
            for(i=0; i<reminds.length; i++){
                reminds[i].timestamp = new Date(reminds[i].time).getDate()-localOffset;
...
//        }
        ...
        $scope.addTime = function() {
                if (firstClick) {
                    $scope.remind.time1Added = true;
                    firstClick = false;
                } else {
                    $scope.remind.time2Added = true;
                }
//                document.getElementById("addtime").style.display = 'block';
//                "<label class='item item-input'><div class='input-label positive'> Pick a Time </div><input type='time' ng-model='remind.time1'></label>";
            }
        ...
        $scope.saveRemind = function () {
            ...
            var inputDate = new Date($scope.remind.start).getTime() - localOffset;
            var inputTime1 = new Date($scope.remind.time1).getTime() - localOffset;
            var inputTime2 = new Date($scope.remind.time2).getTime() - localOffset;

            if(inputDate < nowTime){
                var alertPopup = $ionicPopup.alert({
                    title: 'Start date invalid',
                    template: "Please pick a start date no earlier than today"
                });
                alertPopup.then(function(res) {
            ...
            babyThingsApp.addNotification(notificationRemindId, $scope.remind.title, $scope.remind.dosage, $scope.remind.content, inputTime1, inputTime2, inputDate);
 ...
 */
    .controller('MyCtrl', function ($scope, $state, $location, $ionicPopup, $timeout, $http, Categories, Things, Records) {
        ...
        $scope.testNotification = function(){
            babyThingsApp.addNotification(999,'title','message','dosage',new Date(new Date().getTime()+10000));
        }
